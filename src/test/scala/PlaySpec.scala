...
class PlaySpec extends org.specs2.mutable.Specification {
  ...
  def running[T](app: => play.api.Application)(f: => T): T =
    Helpers.running(app)(f)

  def running[T]()(f: play.api.Application => T): T = {
    val app = FakeApplication()
    Helpers.running(app)(f(app))
  }

  def running[T](cf: GuiceApplicationBuilder => GuiceApplicationBuilder = identity[GuiceApplicationBuilder])(f: play.api.Application => T): T = {
    val app = cf(new GuiceApplicationBuilder()).build
    running(app)(f(app))
  }
